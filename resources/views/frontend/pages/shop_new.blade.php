        ...
        <div class="row">
            <div class="col-lg-12">
                <ul class="breadcrumb-list">
                    ...
                    <li class="breadcrumb-item active">Hàng mới về</li>
                </ul>
            </div>
        </div>
    </div>
                ...
                <div class="shop-top-bar mt-95">
                    <div class="shop-bar-inner">
                        ...
                        <div class="toolbar-amount">
                            <span>Showing {{ $product->firstItem() }} to {{ $product->lastItem() }} of
                                    {{ $product->total() }}
                                    ({{ $product->lastPage() }} Pages)</span>
                        </div>
                    </div>
                        ...
                        <div class="paginatoin-area">
                            <div class="row">
                                <div class="col-lg-6 col-md-6">
                                    <p>Showing {{ $product->firstItem() }} to {{ $product->lastItem() }} of
                                    {{ $product->total() }}
                                    ({{ $product->lastPage() }} Pages)</p>
                                </div>
                                <div class="col-lg-6 col-md-6">
                                    <ul class="pagination-box">
                                        @if ($product->currentPage() > 1)
                                        <li><a class="Previous" href="{{ $product->previousPageUrl() }}"><i class="fa fa-chevron-left"></i> Previous</a>
                                        @endif
                                        </li>
                                        @for($i=1; $i<$product->lastPage(); $i++)
                                        <li class="active"><a href="{{$product->url($i)}}">{{$i}}</a></li>
                                        @endfor
                                        @if ($product->currentPage() < $product->lastPage())
                                        <li>
                                          <a class="Next" href="{{$product->nextPageUrl()}}"> Next <i class="fa fa-chevron-right"></i></a>
                                        </li>
                                        @endif
                                  </ul>
                              </div>
